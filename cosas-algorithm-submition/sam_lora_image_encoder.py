...
class LoRA_Sam_v0_v2(nn.Module):
    ...
    def __init__(self, sam_model: Sam, r: int, lora_layer=None):
        super(LoRA_Sam_v0_v2, self).__init__() # Inheritance and initialization of the nn.Module
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            # Additional conv_se_q layers defined above in this python script
    ...
    def load_lora_parameters(self, filename: str) -> None:
        ...
        # GPU is available, load the state dict as before
            state_dict = torch.load(filename)
        else:
            # GPU is not available, load the state dict on CPU with weights_only=True
